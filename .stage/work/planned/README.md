...
`work/current/`로 물리적으로 이동하며(`scripts/start_work.py`), 종결되면
`official/work/archive/items/`에 안착한다 — 보드 위를 이동하는 카드처럼. 계획된 카드는 기획,
...
- 계획 상태: `captured`, `triaged`, `ready`, `selected`, `deferred`, `rejected`. 작업 시작은
  ...
  W-00000001 --scope "..."`가 카드를 `work/current/`로 옮기고 `active`로 설정하며,
