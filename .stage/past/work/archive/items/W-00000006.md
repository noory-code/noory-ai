...
scope: stage/skills, stage/scripts, stage/hooks, stage/docs, stage/templates, stage/CHANGELOG.md, stage/.claude-plugin/plugin.json, stage/.codex-plugin/plugin.json, .stage/present/work/retrospectives/R-00000005.md
promotes:
decision_refs:
---
...
`past/work/archive/` writes need a *new* open work item (a "registration gate") on top of the
...
`source_registration_blocker` returns empty for any `.stage/past/...` target. Archive writes pass
...
- Correct the false "two gates" learning in `.stage/present/work/retrospectives/R-00000005.md`.
...
- Regression test locking "`.stage/past/work/archive` writes need only an archive intent, never a
...
records, not this repo's `.stage/past/` canon.
