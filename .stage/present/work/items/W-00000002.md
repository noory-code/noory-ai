...
- Documented the field in `items/README.md` and `operations/artifacts.md`;
  ...
  `past/canon/vocabulary.md`.
- Bumped both plugin manifests 0.12.1 -> 0.13.0 and added a CHANGELOG entry.
- Synced the live `.stage/present/work/items/_template.md` so new items in this
  repo carry the field. The live `past/canon/vocabulary.md` routing table is
...
not in this repo's `.stage/past/` canon, so there is nothing to promote here.
