...
0.58.0 → 0.59.0. 쌓여 있던 여섯 묶음에 그 버전 제목이 붙었고 두 매니페스트가 같은 값을
말한다. 커밋 `638ea718` 로 푸시 완료.

**쓸 수 있는지 확인했다.** 원격의 매니페스트가 0.59.0 을 말한다. 코덱스 캐시는 처음에
0.58.0 에 머물렀고(P-00000001 이 적은 알려진 문제), 코덱스를 한 번 돌리자 0.59.0 을 집었다 —
사전 점검이 통과로 바뀐 것으로 확인했다. 이 기계의 클로드 쪽은 stage 를 이 저장소 디렉터리에서
바로 읽으므로 이미 새 내용으로 돈다. 원격 git 을 원본으로 삼는 프로젝트의 캐시 갱신은 이
세션 밖이라 확인 못 했다.
