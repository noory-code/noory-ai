...
(``fromJson`` / ``toJson`` / invariant boundary, ``plot-entity-template``)
...
def generate_wire_ts() -> str:
    ...
    blocks: list[str] = [
        "// AUTO-GENERATED by plot_mcp/ts_codegen.py (noory-ai engine) — DO NOT EDIT.",
        "// Regenerate (cross-repo, from noory-ai/plot):",
        "//   PLOT_VIEWER_ROOT=<this app>/viewer uv run python -m plot_mcp.ts_codegen",
