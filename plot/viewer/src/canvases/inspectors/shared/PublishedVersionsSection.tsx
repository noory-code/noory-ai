...
export function PublishedVersionsSection({
  ...
  return (
    <section className="mt-4 border-t border-slate-200 pt-3">
      ...
      {error && (
        <div className="rounded border border-rose-300 bg-rose-50 p-2 text-[11px] text-rose-800">
          {error}
        </div>
      )}
          ...
          {versions.map((v) => (
            <li key={v.version}>
              <button
                ...
                className="flex w-full items-baseline justify-between rounded px-2 py-1 text-left hover:bg-slate-100"
              >
                <span className="font-mono text-xs font-semibold text-slate-800">
                  {v.version}
                </span>
                <span className="ml-2 flex-1 truncate text-[11px] text-slate-500">
                  {v.published_at ?? ""}
                </span>
                {v.sha && (
