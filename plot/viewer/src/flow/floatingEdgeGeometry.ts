...
/** Point on `node`'s rectangle border facing `target`'s centre. */
export function nodeBorderPoint(node: NodeRect, target: NodeRect): { x: number; y: number } {
...
export function floatingEndpoints(
  ...
  const s = nodeBorderPoint(source, target);
  const t = nodeBorderPoint(target, source);
