...
| **Context injection (D-2026-06-15-A, in-app)** | Each turn carries the active canvas + the live node selection. The viewer sends `selection` (`[{id, kind, label}]`) on `POST /api/chat/send`; the engine prepends a `[Plot context] Active canvas: <scope>. Selected (N): …` preamble to the CLI message so "fix this" resolves against the selection. Per-turn snapshot; `project` scope injects nothing; selection capped at 20 detailed nodes (rest = ids only). In-app only — the MCP path is a follow-up. (CHAT_ARCH.md Layer 2.) |
| **Conversation scope (D-2026-06-13-H; Layer 1 per-instance D-2026-06-15-B)** | Chat threads are partitioned per canvas scope — `foundation` / `actors` / `services` / `service_detail` — plus one shared `project` scope. `service_detail` is **per-service-instance**: the wire scope carries the service id as `service_detail:<service_id>`, so each service-detail canvas keys its own thread (the scope set equals `CanvasKey ∪ {project}`). The dock follows the active canvas (the service-detail modal scopes to the open service's `service_detail:<id>`, falling back to `services` when no service is resolved). The scope switcher is a **full thread picker** (D-2026-06-15-E): fixed segments **Foundation · Actors · Services · Project** are always shown, plus a **{ServiceDetail}** segment (after a `|` separator, labelled with the base `service_detail` label) while a service-detail is the active canvas. Selecting a segment switches **only the chat thread** (no canvas navigation); the selection defaults to + follows the active canvas, but a click overrides until the next canvas change. The engine keys sessions on `(workspace, provider, scope)`; the viewer sends the active scope on `POST /api/chat/send` and demultiplexes `chat_stream_event` payloads on their `scope`. `Reset` wipes only the active scope's session; other scopes survive. A send / reset that omits `scope` defaults to `project`; an unknown scope — or bare `service_detail` with no id — is a 400. A deleted service's thread is left orphaned (in-memory, cleared on engine restart). The `ChatScope` base member set is parity-guarded (Python ↔ TS, including the `service_detail:<id>` suffix) by `tests/test_chat_scope_parity.py`. |
| **Per-canvas framing (D-2026-06-15-C, in-app)** | Each turn carries a canvas-appropriate **system framing** prepended ahead of the context preamble. The base scope maps to its VISION phase: Foundation → Discovery (surface / sharpen the essence), Actors / Services → Planning (design the value-creation machinery), Service-Detail → Execution (break the plan into concrete steps). The `project` scope gets no framing. The engine assembles the CLI prompt as **framing → context → user message** (empty parts skipped); a parametric `service_detail:<id>` resolves to the shared `service_detail` framing. Framing is code constants (`_SCOPE_FRAMING`), not `.noory/`-editable. The same framing/preamble logic is shared with the MCP path via `plot_mcp/chat_context.py`. (CHAT_ARCH.md Layer 3.) |
| **MCP-path context (D-2026-06-15-D, primary path)** | The external agent (the user's own CLI connected via the `plot_mcp` sidecar) reads the viewer's live context through a read-only MCP tool `get_viewer_context(project_path)` → `{active_canvas, selection, framing, updated_at, stale, has_viewer}`. It is fed by a viewer→engine push: the viewer `POST`s `/api/viewer/context` `{project_path, scope, selection}` (debounced, from one `App` effect over the lifted `[activeScope, selection]`, plus a ~30s heartbeat). Because the bundled MCP server is a **separate `--mcp-stdio` process** from the HTTP sidecar (D-2026-06-14-A), the two share state via a **filesystem rendezvous file** — a sha256(`plot_root`)-keyed JSON under `tempfile.gettempdir()` (OS temp, ephemeral, atomic write); the HTTP process writes, the MCP process reads. Project-keyed by resolved `plot_root`. `framing` reuses the per-canvas `SCOPE_FRAMING`. **Liveness:** same-machine shared clock — the HTTP process stamps `updated_at`, the MCP tool compares to its own `time.time()`; older than the 90s TTL ⇒ `stale`. The 30s viewer heartbeat keeps an idle-but-open viewer fresh, so `stale` / no file ⇒ `has_viewer: false`, `active_canvas: null`, `selection: []` — the agent never treats stale context as current. **Multi-viewer:** last-writer-wins (later atomic write overwrites), disambiguated by `updated_at`. This is the CHAT_ARCH.md "Scope honesty" follow-up giving the primary path the same context the in-app chat got in Layers 1–3. |
...
| **UI** | The chat dock is the **leftmost of three horizontally-resizable panels** — chat \| project sidebar \| canvas (D-2026-06-14-E, `WorkspacePanels` + `react-resizable-panels`). Widths persist (localStorage `plot:workspaceLayout`); chat + sidebar are collapsible by dragging the separator; the canvas keeps a hard min width. The dock stacks: a **compact provider bar** on top (D-2026-06-14-D — collapsed by default, click to reveal the full `ChatProvidersPanel`) carrying a persistent **connection indicator** (D-2026-06-15-F: filled dot + agent name in a readable colour when connected, hollow dot + "Connect your AI agent" when not — `data-connected` 1/0) so the connected agent is legible without expanding, then the full-picker scope switcher, then the message log frame (`role="log"`), then the message textarea + Send button. claude-code selected → a billing-warning banner shows above the log (D-2026-06-14-B). |
