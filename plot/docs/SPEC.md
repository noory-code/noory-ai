...
| **Model selection (D-2026-06-16-C)** | Per-workspace, per-provider model override, persisted in the `chat-provider` JSON alongside the provider (`{"provider", "model"}`). The dock shows the active model on the provider bar (`Provider · model`) and offers a **model field** under the bar: free-text with a `<datalist>` of per-CLI suggestions. Only the `claude` CLI's own documented aliases (`fable` / `opus` / `sonnet`) are pre-listed; codex / gemini model ids are vendor- and version-specific, so the field stays free-text there (the user types the id their CLI accepts) — Plot does not hardcode a stale list. Empty = the CLI's own configured default. The value is passed to the CLI as `--model <model>` (all three accept it; verified against the installed binaries). Changing the provider clears the model (a model valid for one CLI is meaningless for another). The engine applies it via `ChatProvider.set_model(model)` before each turn. Pinned by `tests/test_chat_model.py` + `viewer/tests/chat-model-selection.test.tsx`. |
