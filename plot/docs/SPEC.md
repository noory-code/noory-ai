...
| **Per-canvas framing (D-2026-06-15-C, in-app)** | Each turn carries a canvas-appropriate **system framing** prepended ahead of the context preamble. The base scope maps to its VISION phase: Foundation → Discovery (surface / sharpen the essence), Actors / Services → Planning (design the value-creation machinery), Service-Detail → Execution (break the plan into concrete steps). The `project` scope gets no framing. The engine assembles the CLI prompt as **framing → context → user message** (empty parts skipped); a parametric `service_detail:<id>` resolves to the shared `service_detail` framing. Framing is code constants (`_SCOPE_FRAMING`), not `.noory/`-editable. In-app only — the MCP path is a follow-up. (CHAT_ARCH.md Layer 3.) |
