...
| **Conversation scope (D-2026-06-13-H; Layer 1 per-instance D-2026-06-15-B)** | Chat threads are partitioned per canvas scope — `foundation` / `actors` / `services` / `service_detail` — plus one shared `project` scope. `service_detail` is **per-service-instance**: the wire scope carries the service id as `service_detail:<service_id>`, so each service-detail canvas keys its own thread (the scope set equals `CanvasKey ∪ {project}`). The dock follows the active canvas (the service-detail modal scopes to the open service's `service_detail:<id>`, falling back to `services` when no service is resolved); a segmented switcher lets the user pin the shared `project` thread (explicit, not automatic) and labels a parametric scope with its base `service_detail` label. The engine keys sessions on `(workspace, provider, scope)`; the viewer sends the active scope on `POST /api/chat/send` and demultiplexes `chat_stream_event` payloads on their `scope`. `Reset` wipes only the active scope's session; other scopes survive. A send / reset that omits `scope` defaults to `project`; an unknown scope — or bare `service_detail` with no id — is a 400. A deleted service's thread is left orphaned (in-memory, cleared on engine restart). The `ChatScope` base member set is parity-guarded (Python ↔ TS, including the `service_detail:<id>` suffix) by `tests/test_chat_scope_parity.py`. |
