...
| **Per-canvas framing + hallucination guard (D-2026-06-15-C → system prompt D-2026-06-24-A, in-app)** | Each turn delivers a canvas-appropriate **system framing** plus a constant **hallucination guard** as an **authoritative system prompt**, not glued into the user message where the model reads it as mere conversation (Lever 2, `docs/idea/chat/01-levers.md`). The guard ("ground every statement in the given context, read the canvas with your Plot MCP tools when you don't know, ask otherwise — never invent project details; resolve 'this' to the selected node") is the load-bearing anti-starvation instruction; it ships on **every** scope, including `project`. The base scope maps to its VISION phase for the framing: Foundation → Discovery, Actors / Services → Planning, Service-Detail → Execution; the `project` scope gets the guard alone. Delivery is per-CLI: **claude** uses the native `--append-system-prompt` flag; **codex** (`exec`, no system-prompt flag) prepends it to the message. The engine assembles the user message as **context header → selected-node details → user text** (framing now lives in the system prompt, not the message). `build_system_prompt` is code constants (`HALLUCINATION_GUARD` + `SCOPE_FRAMING`), not `.noory/`-editable, and the SSOT lives in `plot_mcp/chat_context.py`. In-app only — extending the guard to the MCP path is a follow-up (Phase 3). (CHAT_ARCH.md Layer 3.) |
