...
def workspace(tmp_path: Path) -> Path:
    """A workspace root with `.noory/plot/{project}/` already populated so
    `git add -- .noory/plot/` has something to stage."""
    ws = tmp_path / "alpha"
    ws.mkdir()
    proj = ws / ".noory" / "plot" / "proj-a"
...
def test_tag_snapshot_captures_latest_plot_data(workspace: Path) -> None:
    """Each snapshot reflects the on-disk state of .noory/plot/ at tag time."""
    init_workspace_repo(workspace)
    target = workspace / ".noory" / "plot" / "proj-a" / "project.json"
    target.write_text('{"id":"proj-a","v":1}', encoding="utf-8")
    tag_snapshot(workspace, "v1")
    target.write_text('{"id":"proj-a","v":2}', encoding="utf-8")
    tag_snapshot(workspace, "v2")
    blob = subprocess.run(
        ["git", "show", "v1:.noory/plot/proj-a/project.json"],
        cwd=workspace,
        capture_output=True,
        text=True,
    ).stdout
