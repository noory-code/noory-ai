"""Tests for plot/hooks/pre_commit_gate.py — specifically the
...
def test_reset_complete_against_current_repo() -> None:
    ...
    msg = _GATE.reset_complete_check(staged, _PLOT_ROOT)
...
def test_reset_complete_skips_docs_only_commit() -> None:
    ...
    staged = [
        "plot/docs/SPEC.md",
        "plot/docs/DECISIONS.md",
        "plot/CHANGELOG.md",
    ]
    assert _GATE.reset_complete_check(staged, _PLOT_ROOT) is None
...
@pytest.mark.parametrize(
    "staged",
    [
        ["plot/.claude-plugin/plugin.json"],
        ["plot/hooks/hooks.json"],
