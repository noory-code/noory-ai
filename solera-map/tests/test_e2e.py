...
These tests spawn the actual solera_map HTTP server as a subprocess and
...
- The `python -m solera_map` entry point starts and binds to a port.
...
def live_server(tmp_path: Path) -> Any:
    """Spawn a real solera_map server against a seeded .solera/ workspace."""
...
def test_server_respects_solera_map_port_env(tmp_path: Path) -> None:
