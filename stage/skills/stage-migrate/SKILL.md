...
maintenance journal makes every interruption fail closed.
...
The abort path restores the exact durable pre-migration Stage snapshot and, for a chained v3
migration, then restores the original v3 git transaction. Runtime files not owned by the
migration are preserved. After commit, do not run abort: rollback is `git revert
<migration-commit>`.
