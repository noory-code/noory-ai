...
- `PreToolUse`: blocks `.stage` deletion, unregistered governed-file modification, and official artifact modification without a promotion intent. Matches every tool (`*`): names outside the built-in and settings-registered sets return the early allow, so the settings seam can gate tools the static list cannot know.
