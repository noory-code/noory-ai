...
- A built-in or settings-registered write tool targeting `.stage/` except `.stage/.runtime/`, or targeting `CLAUDE.md`/`AGENTS.md` anywhere in the workspace, gets one purpose reminder per active top-level work item and session. The reminder runs after every other deny-capable check and before promotion-intent consumption, reads its text from the live records, and fails open if its marker cannot be read or written.
...
- `purpose-ack/<session_id>` — the retained top-level work item ID makes the purpose gate fire once per active hierarchy and session; SessionStart removes markers older than a day.
