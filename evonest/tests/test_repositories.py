...
def test_proposal_add_path_traversal_blocked(
    ...
    """Verify that a safe filename is generated when a path traversal attack is attempted."""
    # Title containing a path traversal attempt
    path_str = proposal_repo.add("content", title="../../../etc/passwd")
    path = Path(path_str)
    # Verify the file was created inside the proposals directory
    assert path.exists()
    assert path.is_relative_to(paths.proposals_dir)
    # Verify the filename contains no path separators
