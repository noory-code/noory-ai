...
argument-hint: "[status|init|reset|explain on|off|<text>|ask on|off|<text>|gate on|off] [global]"
---
...
2. Interpret the first argument (ignore a trailing `global`):
   - empty or `status` → Do NOT edit. Report: target file path, whether it exists, the effective `explainEnabled` / `askEnabled` / `gateEnabled` values (default `true` when unset), the `gateModel` (default `haiku`), and whether `explainRules` / `askRules` are customized. Then stop.
   ...
   - `reset` → delete the `explainEnabled`, `askEnabled`, `explainRules`, `askRules`, `gateEnabled`, and `gateModel` keys (restore built-in defaults).
   ...
   - `gate on` → set `"gateEnabled": true`. `gate off` → set `"gateEnabled": false` (turns off the Stop-hook answer-check; the start-of-turn reminder stays).
