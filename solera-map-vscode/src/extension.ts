...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
  outputChannel = vscode.window.createOutputChannel("Solera Map");
  context.subscriptions.push(outputChannel);

  const openCommand = vscode.commands.registerCommand("soleraMap.open", async () => {
...
async function openMap(context: vscode.ExtensionContext): Promise<void> {
  ...
  if (!folders || folders.length === 0) {
    void vscode.window.showErrorMessage(
      "Solera Map: open a folder or workspace first.",
    );
    return;
  }
  ...
  if (chosen.kind === "v3") {
    void vscode.window.showInformationMessage(
      "This project still uses the v3 `workspace/` layout. " +
        ...
        "(solera-map v0.1.x reads both layouts; v0.2.0 will drop the v3 fallback.)",
      "Continue anyway",
    );
  }

  const port = vscode.workspace
    .getConfiguration("soleraMap")
    .get<number>("port", 5170);

  const command = vscode.workspace
    .getConfiguration("soleraMap")
    .get<string>("command", "uv run python -m solera_map");
  ...
  if (!alreadyRunning) {
    ...
    try {
      ...
      void vscode.window.showErrorMessage(
        `Solera Map: failed to start server (${msg}). ` +
          `Check that 'uv' is on PATH and the solera-map package is installed.`,
        "Open Output",
      );
