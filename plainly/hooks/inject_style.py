...
def communication_context(style: str) -> str:
    ...
    return (
        ...
        "1) 동작은 서술어로 적는다. 문장을 쓴 뒤 서술어를 찾는다. 서술어가 "
        "`-이다`·`-있다`·`-하다`뿐이면 동작을 명사로 적었는지 확인한다. 동작을 나타내는 동사를 "
        "찾아 서술어로 바꾼다. 명사가 세 개 넘게 이어지면 그중 동작을 나타내는 말이 있는지도 "
        "확인한다. `-것`·`-함`·`-음`·`-화`·`-성`을 여러 번 썼을 때도 같은 검사를 한다.\n"
