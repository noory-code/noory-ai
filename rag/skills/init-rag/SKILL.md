...
> The project root is resolved relative to `${CLAUDE_PROJECT_DIR}` (auto-injected by Claude Code). Identical behavior on macOS · Windows.
...
2. **Source-candidate scan**: from the project root, glob for the following candidates and present only the ones that exist to the user.
   ...
   - If any are found, use AskUserQuestion to choose among `[candidates to add (multiSelect), use only .noory/rag/raw/]`.
...
6. **First-indexing offer**: via AskUserQuestion, "Run the first `rag-reindex` now?" — on Yes, invoke the `/rag:rag-reindex` flow as-is.
...
- `rag_set_settings` returns a `RAG_PROJECT_ROOT not set`-type error → guide to restart Claude Code or `/plugin update rag@noory-ai`.
