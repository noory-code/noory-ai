...
# mashbill-entity-template — adding a new ``kind`` end-to-end
...
[`mashbill-feature-tdd`](../mashbill-feature-tdd/SKILL.md). New kind = new
...
Pattern (from [`Mission.ts`](../../novel/viewer/src/domain/Mission.ts)):
...
LOC ceiling 100 per `mashbill/CLAUDE.md` Gate 2.
...
```bash
cd novel/viewer && npx vitest run -t "NODE_RENDERERS includes {kind}"
cd novel/viewer && npx vitest run -t "inspector registry includes {kind}"
```
...
on drift. Per [D-2026-05-11-D](../../mashbill/docs/DECISIONS.md) — Novel is a
...
Per [Phase D of this batch](../../mashbill/docs/DECISIONS.md). The test
...
```bash
cd novel/viewer && npx tsc --noEmit
cd novel/viewer && npx vitest run
cd mashbill && uv run pytest
cd mashbill && uv run mypy plot_mcp/
cd mashbill && uv run ruff check plot_mcp/ tests/
cd mashbill && uv run pytest tests/test_pre_commit_gate.py        # kill-switch
cd mashbill && uv run pytest tests/test_schema_parity.py          # schema-parity
```

Plus Gate 3 (hands-on browser) per `mashbill/CLAUDE.md` — the new kind
...
1. Bump `mashbill/.claude-plugin/plugin.json` patch / minor.
2. `mashbill/CHANGELOG.md` — Added section listing every file in the
   14-step walk.
3. `mashbill/docs/DECISIONS.md` — `D-YYYY-MM-DD-X` entry with:
...
4. `mashbill/docs/SPEC.md` + `CONCEPTS.md` already updated in Steps 1-2.
...
- [`mashbill-domain-design`](../mashbill-domain-design/SKILL.md) — *before*
...
- [`mashbill-feature-tdd`](../mashbill-feature-tdd/SKILL.md) — the upstream
  pipeline that calls into this skill when the feature is "a new
  kind".
- [`mashbill-design-red-team`](../mashbill-design-red-team/SKILL.md) — run
